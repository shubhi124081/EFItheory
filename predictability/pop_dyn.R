...
wpe <- matrix(0, nrow = ncol(e), ncol = 1)

for (i in seq_len(ncol(e))) {
  stable <- e[, i]
  od <- ordinal_pattern(stable, dim = 3)
  pe[i, ] <- permu_entropy(od, dim = 3)
  wpe[i, ] <- WPE(stable, dim = 3)
}
...
wpe_m <- matrix(rep(wpe, each = t))

df <- as.data.frame(cbind(r_m, e_m, pe_m, wpe_m))
colnames(df) <- c("GrowthRate", "N", "PermutationEntropy", "Weighted_PE")
